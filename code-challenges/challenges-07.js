...
const objLat = (obj) => {
    ...
    const formattedFirstName = obj.firstName.charAt(0).toUpperCase() + firstName.slice(1);
    const formattedLastName = obj.lastName.charAt(0).toUpperCase() + lastName.slice(1);
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (yearsOfExperience > 1) {
            newArr.push({ fullName, tech });//shorthand property notation,use variables with the same name as the property names
        }
    }
    return newArr;
    // }
...
const applicationsStatics = (arr) => {
    ...
    for (let i=0; i < arr.length; i++) {
        const { firstName, lastName, yearsOfExperience, tech } = arr[i];
        if(firstName===null&&arr[i].lastName===null||arr[i].yearsOfExperience<=1)
        {
...
const classesAvg = (data) => {
    const{schoolName,capacity,grades}=data
    for(let i=0;i<grades.length;i++)
    {
        let{grade,numberOfClasses,classes}=grades[i];
        for(let j=0;j<numberOfClasses;j++)
        {
            let{avg,classNumber,classScores}=classes[j];
            let sum=0;
            for(let z=0;z<classScores.length;z++)
            sum+=classScores[z];
